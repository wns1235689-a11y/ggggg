...
## 6.5 부록 — 다설문 매니페스트 확장 (2026-08-09 승인, 별도 스코프)

게이트C와 무관한 신규 설문(예: 월드컵 로봇 인지 거리설문)에 하니스를 재사용하기 위해
`surveys/<id>/manifest.json` + `survey_registry.py` 라우팅 계층을 추가했다(계약은
`surveys/README.md`). **엔진 불변 원칙과의 관계**: 게이트C 계보 파일은 무수정 —
하니스의 화이트리스트·저널 스니핑·판정 매핑 6개 지점만 레지스트리 조회로 치환했고,
`surveys/gatec_v23/manifest.json`이 기존 스크립트를 가리켜 기존 런의 동작은 동일하다
(회귀 확인: R3 단일풀·다풀 런의 진단·판정·리포트 산출 불변). §5.5의 "문항 편집 UI
금지"는 유지 — 신규 설문도 코드(파일)로만 정의한다.
