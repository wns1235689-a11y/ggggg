export const meta = {
  name: 'gateC-vs-v3',
  description: 'v3-sim 보정 엔진: v2.3 문항·스키마·프롬프트 골격 그대로 계승 + 2요인(과거경험부정·해결지향) 특성 노출. 결합은 샘플러가 주입(부록A A11), 어느 특성→어느 보기 매핑은 여전히 미지정(방향 미주입 유지)',
  phases: [{ title: 'V3', detail: 'v3-sim 2요인 페르소나 전 문항 분포(한 페르소나=한 에이전트)' }],
}
...
[유형 pid=${p.pid}] ${p.S1}·${p.S2}·${p.S3}·간편식 ${p.S4}·동남아여행 ${p.S5}. 향신료부담 ${lvl(p.향기피)}·매실청친숙 ${lvl(p.매실청)}·관여도 ${lvl(p.관여)}·회의도 ${lvl(p.회의)}·가격민감 ${lvl(p.가격민감)}·접근성장벽 ${lvl(p.접근성)}·정통성기대 ${lvl(p.정통기대)}·식사량기대 ${lvl(p.식사량)}·이 카테고리 섭취빈도 ${lvl(p.카테고리빈도)}·과거 이 카테고리(팟타이·동남아식) 경험에 대한 부정적 평가 ${lvl(p.과거경험부정)}·장벽 해소 시 거부 소거 성향 ${lvl(p.해결지향)}.
...
const out = (await parallel(personas.map(p => () =>
  agent(prompt(p), { label: `v3:pid${p.pid}`, phase: 'V3', schema: SCHEMA, effort: 'medium' })
))).filter(Boolean)
log(`v3-sim 분포 생성: ${out.length}/${personas.length}`)
return { v3: out }
