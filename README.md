...
> **문서 지도 (읽는 순서)**: [`게이트C_시뮬결과_정리.md`](게이트C_시뮬결과_정리.md)(시뮬 산출 결과) → [`게이트C_대조보고서.md`](게이트C_대조보고서.md)(파일럿 실측 대조) → [`SPEC_V3.md`](SPEC_V3.md)(보정 시뮬 사양·사전등록 판정 기준) → [`게이트C_보정시뮬_보고서.md`](게이트C_보정시뮬_보고서.md)(**최종 판정: 부적합 2/4 · 널 79.8%**).
>
> 종결 지점: 커밋 **`3e3e563`** · 태그 **`gateC-complete`** — 태그는 이 세션에서 생성했으나 **원격 푸시가 정책상 차단(HTTP 403)** 되어 로컬에만 있다. 원격에 남기려면 로컬에서 `git tag -a gateC-complete 3e3e563 && git push origin gateC-complete`.
