...
> **문서 지도 (읽는 순서)**: [`게이트C_시뮬결과_정리.md`](게이트C_시뮬결과_정리.md)(시뮬 산출 결과) → [`게이트C_대조보고서.md`](게이트C_대조보고서.md)(파일럿 실측 대조) → [`SPEC_V3.md`](SPEC_V3.md)(보정 시뮬 사양·사전등록 판정 기준) → [`게이트C_보정시뮬_보고서.md`](게이트C_보정시뮬_보고서.md)(**최종 판정: 부적합 2/4 · 널 79.8% · 태그 `gateC-complete`**).
